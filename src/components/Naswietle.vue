<template>
  <v-group :config="{ x: left, y: top }">
    <div v-if="['D1N01', 'D1N02', 'D1N03', 'D1N04', 'D1N05', 'D1N06', 'D1N07'].indexOf(product.typ) >= 0">
      <v-image id="okleina" :config="{ image: okleinaimage, width: width, height: height, crop: { width: width * 2, height: height * 2, x: 0, y: 10 } }"></v-image>
      <v-image id="lewe" :config="{ image: nl, y: grubosc, width: grubosc, height: height - 2 * grubosc, crop: { width: 90, height: 272, x: 0, y: 0 } }"></v-image>
      <v-image id="prawe" :config="{ image: nl, x: width, height: height - 2 * grubosc, y: grubosc, width: grubosc, scaleX: -1, crop: { width: 90, height: 110, x: 0, y: 0 } }"></v-image>
      ...
      <v-image id="prawegora" :config="{ image: nlt, x: width, height: grubosc, width: grubosc, scaleX: -1, crop: { width: 90, height: 90, x: 0, y: 0 } }"></v-image>
      <v-image id="prawedol" :config="{ image: nlt, x: width, height: grubosc, width: grubosc, scaleX: -1, scaleY: -1, y: height, crop: { width: 90, height: 90, x: 0, y: 0 } }"></v-image>
      <v-image
        id="tlo"
        :config="{ image: okleinaimage5, x: grubosc, y: grubosc, width: width - 2 * grubosc, height: height - 2 * grubosc, crop: { width: (width - 2 * grubosc) * 4, height: (height - 2 * grubosc) * 4, x: (s + grubosc - width + (500 - this.product.szerokoscnb2) / 4) * 4, y: 500 } }"
      ></v-image>
      <v-image
        v-if="['Pz', 'Lw'].indexOf(this.product.kierunek) >= 0"
        id="grafika"
      ...
      <v-image
        v-else
        id="grafika"
        :config="{ image: okleinaimage4, x: grubosc, y: grubosc, width: width - 2 * grubosc, height: height - 2 * grubosc, crop: { width: (width - 2 * grubosc) * 4, height: (height - 2 * grubosc) * 4, x: grubosc * 4 + (500 - this.product.szerokoscnb1), y: 500 } }"
      ></v-image>
      <!-- <v-image id="grafika" 
        :config="{ image: okleinaimage4, x: grubosc, y: grubosc, width: width - 2 * grubosc, height: height - 2 * grubosc, 
        crop: { width: (width - 2 * grubosc) * 4, height: (height - 2 * grubosc) * 4, x: (s + grubosc - width+(500-this.product.szerokoscnb2)/4) * 4, y: 500 } }"
      ></v-image> -->
      <v-image id="ramka glass" :config="{ image: okleinaimage2, x: grubosc, y: grubosc, width: width - 2 * grubosc, height: height - 2 * grubosc }" turnable="false"></v-image>
      <!-- <v-image id="LED" :config="{ image: LED, x: grubosc, y: 180 / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image> -->
      <!-- <v-image id="LED" :config="{ image: LED, x: grubosc, y: (180 + 270) / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image>
      <v-image id="LED" :config="{ image: LED, x: grubosc, y: (180 + 2 * 270) / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image>
      <v-image id="LED" :config="{ image: LED, x: grubosc, y: (180 + 3 * 270) / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image>
      <v-image id="LED" :config="{ image: LED, x: grubosc, y: (180 + 4 * 270) / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image>
      <v-image id="LED" :config="{ image: LED, x: grubosc, y: (180 + 5 * 270) / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image>
      <v-image id="LED" :config="{ image: LED, x: grubosc, y: (180 + 6 * 270) / 4, width: width - 2 * grubosc, height: 20 / 4 }" turnable="false"></v-image> -->
      <!-- <div v-for="i in leds[0].ilosc" :key="i"> -->

      <div v-if="this.product.szyba2 == 'o4piaskPWzL'">
        <v-image
          id="LED"
          :config="{ image: LED2, x: grubosc + margines, y: grubosc, width: width - 2 * grubosc, height: height - 2 * grubosc, crop: { width: (width - 2 * grubosc) * 4, height: (height - 2 * grubosc) * 4, x: (s + grubosc - width + (500 - this.product.szerokoscnb2) / 4) * 4, y: 500 } }"
        ></v-image>
      ...
      <v-line id="lg" :config="{ points: [0, 0, grubosc, grubosc, width - grubosc, grubosc, width, 0, 0, 0, 0, height, grubosc, height - grubosc, width - grubosc, height - grubosc, width, height, width, 0], stroke: 'black', strokeWidth: 0.3 }"></v-line>
    ...
    <div v-if="['D1N08'].indexOf(product.typ) >= 0">
      <v-image id="okleina" :config="{ image: okleinaimage, width: width + 25 / 4, x: -25 / 4, height: height, crop: { width: width * 2, height: height * 2, x: 10, y: 10 } }"></v-image>
      <v-line id="lg" :config="{ points: [-25 / 4, 25 / 4, width, 25 / 4, width, height, -25 / 4, height, -25 / 4, 25 / 4], stroke: 'black', strokeWidth: 0.9 }"></v-line>
    </div>
  </v-group>
</template>
...
export default {
  props: ["width", "height", "s", "grubosc", "back", "turnable", "fill"],
  watch: {},
  data() {
    return {
      margines: 0,
  ...
  methods: {
    ...
    drawOkleina2: function() {
      // {wzory:['12C'],ilosc:7,gora:180,odstep:270}
      ...
      if (this.product.szyba2 == "p2" || this.product.szyba2 == "p3") {
        kolor = this.product.szyba2;
      }
