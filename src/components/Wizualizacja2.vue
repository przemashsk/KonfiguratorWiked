<template>
  <div class="myrow" id="wizcon" style="transition:1s;opacity:1;position:relative;">
    ...
    <v-stage :config="{width:widthtc,height:2080*0.5,draggable:false,zoombool:true}">
...
import drzwi from "@/components/Drzwi2.vue";
...
export default {
  ...
  computed: {
    ...
    heightc() {
      let HEIGHT = 2080*0.5;
      if (this.product.typ == "D1N04" || this.product.typ == "D1N05" || this.product.typ == "D1N06" || this.product.typ == "D1N07" || this.product.typ == "D1N08") {
    ...
    widthtc() {
      ...
      if (this.product.typ == "D1N01") {
        ...
        return (WIDTH + this.szerokoscnb1+ this.szerokoscnb2)*(HEIGHT/(2080*0.5+this.wysokoscng));
