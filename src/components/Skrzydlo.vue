  ...
  <v-group :config="{ x: left, y: top }">
    <v-image
      ...
      :config="{
        ...
        height: height - 3,
        image: okleinaimage,
        crop: { width: width * 3, height: height * 3, x: 500, y: 10 },
      }"
    ></v-image>
    <v-image
      id="mixokleina"
      v-if="
        mixbool &&
        this.product.wzor != 'FL1' &&
        this.product.wzor != 'FL1A' &&
        this.product.wzor != 'FL2'
      "
      :config="{
        y: -3,
    ...
    <v-group
      v-if="
        ...
        this.product.kierunek.indexOf('w') >= 0 &&
        selectedwidok == 'Z'
      "
    >
      <v-image
    ...
    <v-image
      id="szyba2"
      :config="{ y: -3, width: width, height: height - 3, image: szybaimage2 }"
    ></v-image>
    <v-image
      id="szyba"
      :config="{ y: -3, width: width, height: height - 3, image: szybaimage }"
    ></v-image>
    <v-image
      id="tloczenie"
      :config="{
        ...
        width: width,
        height: height - 3,
        image: tloczenieimage,
      }"
      v-if="
        (this.selectedwidok != 'Z' &&
          (this.activeModel.typ == 'GD' ||
            this.activeModel.typ == 't' ||
            this.activeModel.typ == 'n')) ||
        (this.activeModel.typ != 'GD' &&
          !['12A', '12B', '12C', '3D3', '3D4', '3D2', '3D1'].includes(
            this.product.wzor
          ))
      "
    ></v-image>
    ...
    <v-image
      ...
      v-if="
        this.product.wzor == '12A' ||
        this.product.wzor == '12B' ||
        this.product.wzor == '12C'
      "
    ></v-image>
    ...
    <v-image
      ...
      :config="{
        ...
        width: width,
        height: height - 3,
        image: tloczenieimage3d,
      }"
      v-if="this.activeModel.typ == 'MG' || this.product.wzor == '51'"
    ></v-image>
    ...
    <v-image
      v-if="
        (this.selectedwidok == 'Z' && this.product.kopniak == '2') ||
        (this.selectedwidok == 'W' && this.product.kopniak == '1') ||
        this.product.kopniak == '3'
      "
      id="tloczeniee"
      :config="{ y: -6, width: width, height: height, image: tloczenieimagee }"
    ></v-image>
    <v-image
      v-if="this.selectedwidok == 'Z' && this.product.automatyka != '-'"
      id="automatyka"
      :config="{ width: width, height: height, image: automatykaImage }"
    ></v-image>
    <v-image
      id="klamka"
      :config="{
    ...
    <v-image
      id="rozeta górna"
      :config="{ y: 0 / 4, width: width, height: height, image: rozetaimage }"
      v-if="this.product.seria != '41'"
    ></v-image>
    ...
    <v-image
      ...
      :config="{
        ...
        width: width,
        height: height,
        image: rozetaimagedol,
      }"
      v-if="
        (this.product.sposobotw == 'KP' &&
          (this.product.klamkaWew == 'Lava' || this.selectedwidok == 'Z')) ||
        this.product.sposobotw == 'PP'
      "
    ></v-image>
    <v-image
      id="zawiasy"
    ...
    <v-image
      id="klamka3"
      :config="{ y: 0 / 4, width: width, height: height, image: klamkaimage3 }"
    ></v-image>
    <v-image
      id="klamka4"
      :config="{ y: 0 / 4, width: width, height: height, image: klamkaimage4 }"
    ></v-image>
    <v-image
      v-if="this.product.progled && this.selectedwidok == 'Z'"
      id="prog_led"
      :config="{ y: 0 / 4, width: width, height: height, image: prog_led }"
    ></v-image>
    ...
    <v-image
      v-if="
        ...
        this.product.sposobotw != 'KG' &&
        selectedwidok == 'Z'
      "
      id="Pierścień"
      :config="{ y: 0 / 4, width: width, height: height, image: klamkaimage5 }"
    ></v-image>
    <v-rect
      id="Obramówka"
      :config="{
...
export default {
  ...
  methods: {
    ...
    drawKlamka: function () {
      this.product.wzor == "3D2" &&
      this.selectedwidok == "Z" &&
      this.product.klamka.indexOf(120) > -1
        ? (this.korektay = 240)
        : (this.korektay = 0);
      ...
      if (
        this.product.seria == "41" ||
        this.product.seria == "661" ||
        this.product.wzor == "GF6"
      ) {
      ...
      if (this.product.klamka.indexOf("x") >= 0 && this.selectedwidok == "Z") {
        imageObj.src =
          ...
          folder +
          "/" +
          this.klamkac.substring(0, this.klamkac.length - 1) +
          "_" +
          this.product.klamkakolor +
          ".png";
      } else {
        imageObj.src =
          "/images/" +
        ...
        if (this.product.sposobotw == "PP" && this.selectedwidok == "W") {
          imageObj5.src =
            "/images/" + folder + "/solo_" + this.product.klamkakolor + ".png";
        }
      }
      // console.log(imageObj.src);
      ...
      if (
        this.product.klamka.indexOf("SS") >= 0 ||
        this.product.klamka.indexOf("SD") >= 0 ||
        this.product.klamka.indexOf("SR") >= 0 ||
        (this.product.klamka.indexOf("SL") >= 0 &&
          this.product.klamka.indexOf("Pock") == -1 &&
          this.product.klamka != "PockSLT20")
      ) {
        if (
          (this.selectedwidok == "Z" && this.product.sposobotw == "KP") ||
          this.product.sposobotw == "PP"
        ) {
          imageObj.src = "/images/tloczenia/01.webp";

          imageObj2.src =
            `/images/SPACE/` + folder2 + this.product.klamkakolor + `.png`;
          // console.log()
          imageObj3.src =
            `/images/SPACE/` +
            this.klamkac.substring(2, 4) +
            this.klamkac.substring(8, 10) +
            `.png`;
      ...
      if (
        this.product.seria != "661" &&
        ((this.product.kierunek.indexOf("z") >= 0 &&
          this.selectedwidok != "W") ||
          (this.product.kierunek.indexOf("w") >= 0 &&
            this.selectedwidok != "Z"))
      ) {
      ...
      rozetaObjDol.src =
        `/images/rozety dol/` + rozeta2 + `_` + rozetakol + `.png`;
    ...
    drawSzyba2() {
      ...
      //   } else {
      if (
        this.product.wzor == "47A" ||
        this.product.wzor == "47" ||
        this.product.wzor == "48" ||
        this.product.wzor == "50"
      ) {
        imageObj.src =
          "/images/baza/" +
          this.product.wzor +
    ...
    drawSzyba() {
      ...
      if (this.selectedwidok == "W" && this.activeModel.typ == "GL") {
        ...
        if (litera == "A" || litera == "C") {
          imageObj.src =
            "/images/szyby/" + "GLA" + "_" + this.szybac + "_wew.png";
        } else {
          imageObj.src =
            "/images/szyby/" + "GL" + "_" + this.szybac + "_wew.png";
        }
        // alert(imageObj.src);
      } else {
        // imageObj.src = '/images/naswietla/nn.jpg';
        if (this.product.szyba == "00") {
          ...
          imageObj.src =
            "/images/szyby/" + tyl + this.odpszyb + "_" + this.szybac + ".png";
          // }
        }
      }
      // console.log(imageObj.src);
    ...
    // console.log('ertyui')
      ...
      if (this.product.inoxkolor == "10304") {
        if (this.product.wzor.substring(0, 2) == "RL") {
          if (
            (this.selectedwidok == "Z" && this.product.kolor.length == 2) ||
            (this.selectedwidok == "W" && this.product.kolor2.length == 2)
          ) {
            file += "_black";
          }
    ...
    drawAutomatyka: function () {
      ...
      automatykaObj.src =
        "/images/automatyka/" + this.product.automatyka + ".webp";
    ...
    drawTloczeniee: function () {
      ...
      imageObj.src =
        "/images/tloczenia/kopniak" + this.product.kopniakkolor + ".png";
      imageObj.onload = () => {
  ...
  computed: {
    ...
    mixc() {
      return this.mixdanetab.find((el) => el.model == this.product.wzor)
        ? this.mixdanetab.find((el) => el.model == this.product.wzor).dane
        : {};
    },
    mixcCorrectX() {
    ...
    mixcCorrectWidth() {
      return this.back == true
        ? this.mixc.width + this.mixc.correctwidth
        : this.mixc.width;
    },
    mixbool() {
      if (this.product.wzor) {
        return this.mixkolorlista.indexOf(this.product.wzor) >= 0 &&
          (this.product.wariant == "M" || this.product.wariant == "MB")
          ? true
          : false;
      } else {
        return false;
    ...
    tloczeniepcv() {
      return this.activeModel.typ == "PCV" && this.product.szyba == "00"
        ? true
        : false;
    },
  },
